...
@test "checkpoints: comment, INIT/AUTO and AGENT_/SKILL_ event lines are excluded" {
    init_project
...
2026-01-01T10:03:00Z | CP_1_002 | Real work saved
EOF
    run hook_context
    [ "$status" -eq 0 ]
...
EOF
    ...
    grep -q "## Phase Transition: requirements -> design" "$h"
    grep -q -- "- \*\*Checkpoint\*\*: CP_1_002 - after migration" "$h"

    # Idempotent: a second refresh keeps exactly one block
