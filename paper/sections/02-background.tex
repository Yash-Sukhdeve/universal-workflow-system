...
\paragraph{Git-Native State} UWS uniquely stores all state in version-controlled YAML files within the project's git repository. This provides:

\begin{itemize}
    \item \textbf{Natural versioning}: Every checkpoint is a git commit
    \item \textbf{Diffing}: State changes are visible via \texttt{git diff}
    \item \textbf{Branching}: Experimental workflows via git branches
    \item \textbf{Collaboration}: Team workflows via standard git operations
    \item \textbf{Familiarity}: Developers already know git
\end{itemize}
